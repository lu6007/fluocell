...
% model_type = 1; % 1 - methylation triggers phosphorylation;
model_type = 2; % 2 - phosphorylatoin triggers methylation; 
num_sims = length(nk);
res = cell(num_sims,1);
for i = 1:num_sims,
    switch model_type
        ...
        case 2, 
            res{i} = phospho_methyl_model(data, 'num_phosphos', nk(i), 'show_figure', 0);
    end;
end;
time = res{1}.time;
...
title('Recruit Model: Triggering Molecule/Histone Ratio');
...
% %% No Recruit, the effect of triggering KDMS/histone ratio
% data = model_init_data('no_recruit');
...
%             res{i} = methyl_phospho_model(data, 'num_kdms', nk(i));
%         case 2, 
%             res{i} = phospho_methyl_model(data, 'num_phosphos', nk(i));
%     end;
% end;
% time = res{1}.time;
% num_points = size(time, 1);
...
% plot(time(index), methyl(index,:), 'LineWidth', 3);
% xlabel('Time (min)'); ylabel('Methylation Level');
% title('No Recruit: Triggering Molecule/Histone Ratio');
% legend(strcat(num2str(nk), 'K:6M'));
% 
% phos = zeros(num_points, num_sims);
% for i = 1:num_sims,
...
% %% Recruit Model, the effect of max KDMS/histone ratio
% data = model_init_data('with_recruit');
% mk = [100; 200; 400];
% num_sims = length(mk);
% res = cell(num_sims,1);
...
% xlabel('Time (min)'); ylabel('Methylation Level');
% title('Recruit Model: Max Molecule/Histone Ratio');
% legend(strcat(num2str(mk), 'K:6M'));
% 
% %% No Recruit, the effect of max KDMS/histone ratio
% data = model_init_data('no_recruit');
% mk = [100; 200; 400];
% num_sims = length(mk);
% res = cell(num_sims,1);
...
% plot(time, methyl, 'LineWidth', 3);
% xlabel('Time (min)'); ylabel('Methylation Level');
% title('No Recruit: Max Molecule/Histone Ratio');
% legend(strcat(num2str(mk), 'K:6M'));


%% Recruit Model, the effect of a1
data = model_init_data('with_recruit');
