...
%num_rois = length(regions);
num_rois = length(regions)-1;
...
total_int = zeros(num_channels, num_images, num_rois);
ave_int = zeros(num_channels, num_images, num_rois);
port_pixels = zeros(num_channels, num_images, num_rois);
...
save(file_intensity, 'time', 'total_int', 'ave_int','num_channels', 'num_rois');
else
    load(file_intensity);
end;
...
%% Plot curves
gray = [0.39 0.47 0.64];
line_name = cell(num_channels, num_rois);
...
for j = 1:num_rois,
[tt cc(:,1)] = cross_correlation(time, total_int(1,:,j)', total_int(2, :,j)'); clear tt;
[tt cc(:,2)] = cross_correlation(time, ave_int(1,:,j)', ave_int(2,:,j)'); clear tt;
...
num_rois = 4;
regions= cell(num_rois,1);
region_bd = cell(num_rois,1);
